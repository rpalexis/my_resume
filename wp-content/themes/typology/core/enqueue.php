...
function typology_load_css() {
	...
	if ( typology_get_option( 'minify_css' ) ) {
		...
		wp_enqueue_style( 'typology-main', get_template_directory_uri() . '/assets/css/min.css', false, TYPOLOGY_THEME_VERSION );
		...
		foreach ( $styles as $id => $style ) {
			wp_enqueue_style( 'typology-' . $id, get_template_directory_uri() . '/assets/css/' . $style, false, TYPOLOGY_THEME_VERSION );
		}
	}
	...
	//Load RTL css
	if ( typology_is_rtl() ) {
		wp_enqueue_style( 'typology-rtl', get_template_directory_uri() . '/assets/css/rtl.css', array( 'typology-main' ), TYPOLOGY_THEME_VERSION );
	}

	//Load WooCommerce css
	if ( typology_is_woocommerce_active() ) {
		wp_enqueue_style( 'typology-woocommerce', get_template_directory_uri() . '/assets/css/typology-woocommerce.css', array( 'typology-main' ), TYPOLOGY_THEME_VERSION );
	}
...
function typology_load_js() {
	...
	if ( typology_get_option( 'minify_js' ) ) {
		...
		wp_enqueue_script( 'typology-main', get_template_directory_uri() . '/assets/js/min.js', array( 'jquery' ), TYPOLOGY_THEME_VERSION, true );
		...
		foreach ( $scripts as $id => $script ) {
			wp_enqueue_script( 'typology-'.$id, get_template_directory_uri().'/assets/js/'. $script, array( 'jquery' ), TYPOLOGY_THEME_VERSION, true );
		}
	}
